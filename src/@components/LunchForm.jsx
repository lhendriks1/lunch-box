...
const LunchForm = ({ lunchBox, setLunchBox, activeLink, setActiveLink, total }) => {
  ...
  const boxOptions = Object.values(OPTIONS).map((box) => {
    return (
      <li role='option' aria-label={box.displayText} onClick={() => selectBox(box.key)} key={box.key}>
  ...
  return (
    <>
      <div className={`form-section ${activeLink !== 0 ? "hidden" : null}`}>
        <h3 id='selectBox' aria-label='my text'>Select Box</h3>
        <ul role='listbox' aria-labelledby='selectBox' className="flex centered">{boxOptions}</ul>
      </div>
      <div
      ...
      <div className={`form-section ${activeLink !== 1 && "hidden"}`}>
        <span onClick={() => setActiveLink(0)} className="back">
      ...
      >
        <span onClick={() => setActiveLink(1)} className="back">
          <ArrowBack fontSize="small" /> BACK
        </span>
        ...
        <Button
          variant="contained"
          color="primary"
          disabled={!lunchBox.box || !lunchBox.address.address}
        >
          Submit Order
        </Button>
      </div>
